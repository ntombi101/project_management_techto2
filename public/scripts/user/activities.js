...
fetch('/user/api/activities') // Returns a Promise for the GET request
  ...
  .then(function (data) { // Display the JSON data appropriately
    ...
    const classList = document.getElementById('activityLog')
    console.log(data)
    // Iterate through all the results from db
    data.forEach(function (dbResult) {
    // Create a new list entry
      const li = document.createElement('LI')
      const liText = document.createTextNode(`${dbResult.initiatedBy} ${dbResult.activity} in ${dbResult.inGroup}  on the ${dbResult.dateAndTime}`)
