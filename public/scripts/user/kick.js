...
fetch('/groups/api/kick') // Returns a Promise for the GET request
  ...
  .then(data => {
    const votes = data
    console.log(data)
    const votesFound = data.filter(found => ((found.kickMember === 'Yes') || (found.kickMember === 'No')))
    ...
    voteCounts = votes.reduce(
      (acc, vote) => (
        (acc[vote.kickMember] = (acc[vote.kickMember] || 0) + 1)
        , acc
      ),
      {}
    )
